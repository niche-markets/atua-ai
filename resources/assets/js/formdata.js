            ...
            form.querySelectorAll('input[type="checkbox"]').forEach((element) => {
                if (element.name) {
                    if (element.checked === false) {
                        if (element.value === '1') {
                            this.append(element.name, '0');
                        } else if (element.value === '0') {
                            this.append(element.name, '1');
                        } else if (element.value === 'true') {
                            this.append(element.name, 'false');
                        } else if (element.value === 'false') {
                            this.append(element.name, 'true');
                        } else if (!element.hasAttribute('value')) {
                            this.append(element.name, '0');
                        }
                    } else {
                        this.append(element.name, element.hasAttribute('value') ? element.value : '1');
                    }
                }
            });
